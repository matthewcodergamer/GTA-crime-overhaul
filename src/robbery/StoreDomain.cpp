...
ClerkPersonality choosePersonality(const StoreTuning& tuning, const std::uint64_t seed) noexcept {
    ...
    for (std::size_t index = 0; index < tuning.personalityWeights.size(); ++index) {
        ...
        if (cursor <= weight) {
            return static_cast<ClerkPersonality>(index);
        }
        cursor -= weight;
    }
    return ClerkPersonality::Compliant;
...
void applyPersonalityStats(ClerkProfile& profile) noexcept {
    ...
    case ClerkPersonality::Cowardly:
        profile.fear = 0.90f;
        profile.compliance = 0.96f;
        profile.alarmTendency = 0.10f;
        profile.resistance = 0.01f;
        profile.withholdingTendency = 0.04f;
        break;
    case ClerkPersonality::Compliant:
        profile.fear = 0.66f;
        profile.compliance = 0.86f;
        profile.alarmTendency = 0.20f;
        profile.resistance = 0.04f;
        profile.withholdingTendency = 0.08f;
        break;
    case ClerkPersonality::Defiant:
        profile.fear = 0.46f;
        profile.compliance = 0.42f;
        profile.alarmTendency = 0.42f;
        profile.resistance = 0.24f;
        profile.withholdingTendency = 0.52f;
        break;
    case ClerkPersonality::Panicky:
        profile.fear = 0.98f;
        profile.compliance = 0.52f;
        profile.alarmTendency = 0.34f;
        profile.resistance = 0.02f;
        profile.withholdingTendency = 0.10f;
        break;
    case ClerkPersonality::Armed:
        profile.fear = 0.42f;
        profile.compliance = 0.50f;
        profile.alarmTendency = 0.24f;
        profile.resistance = 0.72f;
        profile.withholdingTendency = 0.20f;
        break;
    case ClerkPersonality::Experienced:
        profile.fear = 0.54f;
        profile.compliance = 0.74f;
        profile.alarmTendency = 0.72f;
        profile.resistance = 0.12f;
        profile.withholdingTendency = 0.24f;
        break;
    case ClerkPersonality::Reckless:
        profile.fear = 0.32f;
        profile.compliance = 0.34f;
        profile.alarmTendency = 0.46f;
        profile.resistance = 0.66f;
        profile.withholdingTendency = 0.36f;
        break;
...
void PrototypeStoreModel::initializePersistent(
    LogicalIdGenerator& ids,
    const std::uint64_t nowMs,
    const StoreTuning& tuning) {

    if (persistent_.businessId == 0) {
        persistent_.businessId = ids.next(LogicalIdDomain::Business);
    }
...
bool PrototypeStoreModel::ensureReplacementClerk(
    LogicalIdGenerator& ids,
    const std::uint64_t nowMs,
    const StoreTuning& tuning) {

    if (!persistent_.clerkVacant || nowMs < persistent_.replacementEligibleAtMs) {
        return false;
    }
    ...
    persistent_.clerk = makeClerkProfile(
        newId,
        nextGeneration,
        nowMs,
        tuning,
...
void PrototypeStoreModel::markClerkDead(
    const std::uint64_t nowMs,
    const StoreTuning& tuning) {
    ...
    if (hasActiveSession()) {
        abort(RobberyAbortReason::ClerkDied, nowMs);
    }
}
...
void PrototypeStoreModel::clearThreat() {
    if (session_.state == RobberySessionState::ThreatDetected) {
        session_ = {};
    }
}

bool PrototypeStoreModel::threatSustained(
    const std::uint64_t nowMs,
    const std::uint32_t sustainMs) const noexcept {
...
bool PrototypeStoreModel::beginSession(
    ...
    if (persistent_.clerk.id == 0 || !persistent_.clerk.alive
        || crimeId == 0 || caseId == 0
        || (session_.state != RobberySessionState::ThreatDetected
            && session_.state != RobberySessionState::Idle)) {
        return false;
    }
    ...
    session_.lastProgressAtMs = nowMs;
    session_.openingReaction = chooseOpeningReaction(
        persistent_.clerk,
        physicalClerkArmed,
        crimeId ^ caseId ^ persistent_.clerk.id);
    session_.taskRefreshAtMs = nowMs + tuning.taskRefreshMs;

    if (session_.openingReaction == ClerkReaction::SecretAlarm) {
        session_.alarmDueAtMs = nowMs + tuning.secretAlarmDelayMs;
    }
    if (session_.openingReaction == ClerkReaction::PhoneReport) {
...
DemandResult PrototypeStoreModel::issueDemand(
    const StoreDemand demand,
    const std::uint64_t nowMs) {

    DemandResult result{};
    if (session_.state != RobberySessionState::Active || !persistent_.clerk.alive) {
        return result;
    }
    ...
    case StoreDemand::OpenRegister:
        ...
        result.cashSourceIndex = 0;
        result.action = cashDemand(0, nowMs);
        result.accepted = result.action.kind != StoreActionKind::RefuseDemand;
        return result;
    case StoreDemand::OpenSecondRegister:
        result.usesCashSource = true;
        result.cashSourceIndex = 1;
        result.action = cashDemand(1, nowMs);
        result.accepted = result.action.kind != StoreActionKind::RefuseDemand;
        return result;
    case StoreDemand::EmptySafe: {
        auto found = std::find_if(cashSources_.begin(), cashSources_.end(), [](const CashSource& source) {
            return source.safe;
        });
    ...
    case StoreDemand::HandsUp:
        ...
        result.action = StoreAction{StoreActionKind::HandsUp, 0, 0, "explicit hands-up demand"};
        return result;
    case StoreDemand::GetDown:
        result.accepted = true;
        result.action = StoreAction{StoreActionKind::Cower, 0, 0, "explicit get-down demand"};
        return result;
    case StoreDemand::MoveAwayFromAlarm:
        if (!session_.alarmTriggered) {
            session_.alarmOpportunitySuppressed = true;
            session_.alarmDueAtMs = 0;
        }
        result.accepted = true;
        result.action = StoreAction{StoreActionKind::HandsUp, 0, 0, "alarm opportunity suppressed before trigger"};
        return result;
    case StoreDemand::DontMove:
        result.accepted = true;
        result.action = StoreAction{StoreActionKind::HoldPosition, 0, 0, "hold-position demand"};
        return result;
    }
...
std::optional<StoreAction> PrototypeStoreModel::tick(
    const std::uint64_t nowMs,
    const StoreTuning& tuning,
    const bool threatMaintained) {

    if (session_.state != RobberySessionState::Active) {
        return std::nullopt;
    }

    if (nowMs >= session_.startedAtMs
        && nowMs - session_.startedAtMs >= tuning.sessionTimeoutMs) {
    ...
    if (session_.reportDueAtMs != 0 && nowMs >= session_.reportDueAtMs
        && !session_.reportCompleted) {
        session_.reportDueAtMs = 0;
        session_.lastProgressAtMs = nowMs;
        return StoreAction{StoreActionKind::BeginPhoneReport, 0, 0, "report timer completed"};
    }

    if (session_.openingReaction == ClerkReaction::ArmedResistance
        && !session_.clerkFought && !threatMaintained) {
    ...
    if (nowMs >= session_.taskRefreshAtMs) {
        ...
        case ClerkReaction::SecretAlarm:
            return StoreAction{StoreActionKind::HandsUp, 0, 0, "bounded task refresh"};
        case ClerkReaction::Cower:
        case ClerkReaction::PhoneReport:
            return StoreAction{StoreActionKind::Cower, 0, 0, "bounded task refresh/fallback"};
        case ClerkReaction::Flee:
            if (!session_.clerkFled) {
                return StoreAction{StoreActionKind::Flee, 0, 0, "flee reaction refresh"};
            }
            break;
        case ClerkReaction::ArmedResistance:
            if (!session_.clerkFought) {
                return StoreAction{StoreActionKind::HandsUp, 0, 0, "armed clerk waiting for opening"};
            }
            break;
        case ClerkReaction::HoldPosition:
            return StoreAction{StoreActionKind::HoldPosition, 0, 0, "bounded hold refresh"};
        }
    }
...
void PrototypeStoreModel::abort(
    const RobberyAbortReason reason,
    const std::uint64_t nowMs) {

    if (session_.state == RobberySessionState::Idle || session_.state == RobberySessionState::Completed) {
        return;
    }
...
void PrototypeStoreModel::complete(
    const std::uint64_t nowMs,
    const StoreTuning& tuning) {

    int exposed = 0;
    for (const auto& source : cashSources_) {
        exposed += source.offeredAmount;
    }
...
ClerkReaction PrototypeStoreModel::chooseOpeningReaction(
    ...
    case ClerkPersonality::Compliant:
        return roll < 0.18f ? ClerkReaction::Cower : ClerkReaction::HandsUp;
    case ClerkPersonality::Defiant:
        return roll < profile.alarmTendency ? ClerkReaction::SecretAlarm : ClerkReaction::HandsUp;
    case ClerkPersonality::Panicky:
        return roll < 0.50f ? ClerkReaction::Flee : ClerkReaction::Cower;
    case ClerkPersonality::Armed:
        return physicalClerkArmed && roll < profile.resistance
            ? ClerkReaction::ArmedResistance
            : ClerkReaction::HandsUp;
    case ClerkPersonality::Experienced:
        return roll < profile.alarmTendency ? ClerkReaction::SecretAlarm : ClerkReaction::HandsUp;
    case ClerkPersonality::Reckless:
        if (physicalClerkArmed && roll < profile.resistance) {
            return ClerkReaction::ArmedResistance;
        }
...
StoreAction PrototypeStoreModel::cashDemand(
    const std::size_t sourceIndex,
    const std::uint64_t nowMs) {

    if (sourceIndex >= cashSources_.size()) {
        return StoreAction{StoreActionKind::RefuseDemand, sourceIndex, 0, "cash source not configured"};
    }
    ...
    if (source.state == CashSourceState::Empty || source.remainingAmount <= 0) {
        ...
        return StoreAction{StoreActionKind::RefuseDemand, sourceIndex, 0, "cash source already empty"};
    }
    if (source.state == CashSourceState::Unavailable) {
        return StoreAction{StoreActionKind::RefuseDemand, sourceIndex, 0, "cash source unavailable"};
    }

    const float refusalRoll = unitRoll(session_.crimeId ^ session_.clerkId, 0xCA55ull + sourceIndex);
    if (refusalRoll > persistent_.clerk.compliance
        && persistent_.clerk.personality != ClerkPersonality::Cowardly) {
        return StoreAction{StoreActionKind::RefuseDemand, sourceIndex, 0, "personality refusal/stall"};
    }
    ...
    session_.lastProgressAtMs = nowMs;
    return StoreAction{
        StoreActionKind::MoveToCashSource,
        sourceIndex,
        amount,
...
std::optional<ClerkPersonality> clerkPersonalityFromString(const std::string_view value) noexcept {
    for (std::size_t i = 0; i < static_cast<std::size_t>(ClerkPersonality::Count); ++i) {
        ...
        if (clerkPersonalityName(candidate) == value) {
            return candidate;
        }
    }
    return std::nullopt;
